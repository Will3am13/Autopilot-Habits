...
class ScheduleWidget(QWidget):
    def __init__(self):
        ...
        self.start_timer()
        self.events = []
        self.websiteBlocker = WebsiteBlocking()
    ...
    def create_toggleBlock_button(self):
        ...
        self.toggleBlock_button.toggled.connect(self.update_website_blocking)
    ...
    def update_website_blocking(self):
        if self.toggleBlock_button.isChecked():
            self.websiteBlocker.activeBlocking()
        else:
            self.websiteBlocker.inactiveBlocking()
    ...
    def add_schedule(self, schedule):
        ...
        for i, task in enumerate(schedule):
            ...
            response = openai.Completion.create(
                ...
                prompt=prompt,
                max_tokens=250,  # Adjust the max tokens as needed
            ...
            # DEBUG
            print("Raw start_datetime string:", start_datetime)
            print("Raw end_datetime string:", end_datetime)
    ...
    def add_event(self, event):
        ...
        for row in range(start_row, end_row + 1):
            # Adjust the column index for Sunday (6)
            col = end_datetime.date().weekday()

            existing_item = self.table.item(row, col)
            if existing_item:
                existing_event = existing_item.data(PySide6.QtCore.Qt.UserRole)
                QMessageBox.warning(self, "Conflict", f"There is already an event scheduled at this time: {existing_event['title']}")
                return

            event_item = QTableWidgetItem(event['title'])
            event_item.setData(PySide6.QtCore.Qt.UserRole, event)
            if event['color']:
    ...
    def check_current_event(self):
        current_time = PySide6.QtCore.QTime.currentTime()
...
class AddScheduleDialog(QDialog):
    schedule_created = PySide6.QtCore.Signal(list)
    import PySide6
...
class AddEventDialog(QDialog):
    event_created = PySide6.QtCore.Signal(dict)
